...
export default function UserDashboardPage() {
    ...
    return (
      <div className="min-h-screen bg-zinc-50 dark:bg-dark flex items-center justify-center">
        <div className="flex flex-col items-center">
          ...
          <p className="text-xs text-zinc-500 font-bold uppercase tracking-wider">Verifying client credentials...</p>
        </div>
      </div>
    );
  }
  ...
  // AI suggestions for user based on wishlist/sourcing
      ...
      <div className="min-h-screen bg-zinc-50 dark:bg-dark/20 flex flex-col">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 w-full py-12 flex-1 grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          ...
          <div className="lg:col-span-3 bg-white dark:bg-zinc-950 border border-zinc-150/60 dark:border-zinc-850 rounded-3xl p-6 shadow-xs space-y-6">
            ...
            <div className="flex items-center gap-3.5 pb-5 border-b border-zinc-100 dark:border-zinc-900">
              ...
              <div className="text-left leading-tight">
                <h3 className="font-poppins font-bold text-sm text-zinc-900 dark:text-white line-clamp-1">{user.name}</h3>
                <span className="text-[9px] text-zinc-400 font-extrabold uppercase tracking-wide">{user.company || "Procurement Officer"}</span>
              </div>
            </div>
          ...
          <div className="lg:col-span-9 bg-white dark:bg-zinc-950 border border-zinc-150/60 dark:border-zinc-850 rounded-3xl p-6 sm:p-8 shadow-xs min-h-[500px]">
            ...
            {activeTab === "dashboard" && (
              <div className="space-y-8 animate-in fade-in duration-200">
                <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between border-b pb-4 mb-4 gap-2">
                  <div>
                    <h2 className="text-xl font-poppins font-black text-zinc-900 dark:text-white leading-none">Welcome Back, {user.name.split(" ")[0]}</h2>
                    <p className="text-[10px] text-zinc-450 mt-1 font-semibold">Account Category: B2B Verified Sourcing Partner</p>
                  </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
                  <div className="bg-zinc-50 dark:bg-zinc-900/60 border border-zinc-100 dark:border-zinc-850 p-5 rounded-2xl flex items-center justify-between shadow-xs">
                    <div>
                      ...
                      <span className="block text-xl font-poppins font-black text-zinc-900 dark:text-white mt-1">{allInquiries.length} Submitted</span>
                    </div>
                  ...
                  <div className="bg-zinc-50 dark:bg-zinc-900/60 border border-zinc-100 dark:border-zinc-850 p-5 rounded-2xl flex items-center justify-between shadow-xs">
                    <div>
                      ...
                      <span className="block text-xl font-poppins font-black text-zinc-900 dark:text-white mt-1">
                        {subscriptions.filter(s => s.status === "Active").length} Box Plans
                      </span>
                    </div>
                  ...
                  <div className="bg-zinc-50 dark:bg-zinc-900/60 border border-zinc-100 dark:border-zinc-850 p-5 rounded-2xl flex items-center justify-between shadow-xs">
                    <div>
                      ...
                      <span className="block text-xl font-poppins font-black text-zinc-900 dark:text-white mt-1">{giftOrders.length} Bundles</span>
                    </div>
                ...
                <div className="p-6 bg-zinc-50 dark:bg-zinc-900/40 border border-zinc-150/60 dark:border-zinc-850 rounded-2xl space-y-3">
                ...
                <div className="space-y-4">
                  <h3 className="font-poppins font-black text-sm text-zinc-900 dark:text-white uppercase tracking-wider border-b pb-2">AI Recommended for You</h3>
                    ...
                    {aiRecommendations.map((p) => (
                      <div key={p.id} className="flex gap-4 p-3 rounded-2xl border border-zinc-100 dark:border-zinc-850 bg-zinc-50/50 dark:bg-zinc-900/30">
                        ...
                        <div className="flex-1 flex flex-col justify-between">
                          ...
                          <Link href={`/products/${p.slug}`} className="text-[9px] text-primary font-bold uppercase tracking-wider">Configure RFQ &gt;</Link>
                        </div>
                      </div>
                    ))}
                  </div>
            ...
            {activeTab === "wishlist" && (
              <div className="space-y-6 animate-in fade-in duration-200">
                <h2 className="text-xl font-poppins font-black text-zinc-950 dark:text-white border-b pb-3 uppercase tracking-wider">My Wishlist ({wishlist.length})</h2>
                {wishlistToys.length === 0 ? (
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
                    {wishlistToys.map((toy) => (
                      <div key={toy.id} className="relative group">
                        <button
                          onClick={() => toggleWishlist(toy.id)}
                          className="absolute top-4 right-4 p-1.5 rounded-full bg-red-500 text-white z-10 hover:bg-red-650 transition cursor-pointer"
                        >
                        ...
                        <div className="p-3 border-t border-zinc-100 flex justify-center bg-zinc-50 rounded-b-3xl">
                          <Link href={`/products/${toy.slug}`} className="text-[10px] font-bold text-primary uppercase tracking-wider">Configure RFQ</Link>
                        </div>
                      </div>
                    ))}
                  </div>
            ...
            {activeTab === "inquiries" && (
              <div className="space-y-6 animate-in fade-in duration-200">
                <h2 className="text-xl font-poppins font-black text-zinc-950 dark:text-white border-b pb-3 uppercase tracking-wider">Sourcing Inquiries RFQ</h2>
                {allInquiries.length === 0 ? (
                  ...
                  <div className="space-y-4">
                    {allInquiries.map((inq) => (
                      <div
                        key={inq.id}
                        className="p-5 rounded-2xl border border-zinc-150/60 dark:border-zinc-850 bg-zinc-50/50 dark:bg-zinc-900/30 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 hover:border-primary/30 transition"
                      >
                        <div className="space-y-1">
                          ...
                          <p className="text-[10px] text-zinc-450 font-semibold">Submitted: {inq.date} | {inq.totalQuantity} items requested</p>
                ...
                {giftOrders.length === 0 ? (
                  ...
                  <div className="space-y-4">
                    {giftOrders.map((gft) => (
                      <div
                        key={gft.id}
                        className="p-5 rounded-2xl border border-zinc-150/60 dark:border-zinc-850 bg-zinc-50/50 dark:bg-zinc-900/30 space-y-4"
                      >
                        <div className="flex items-center justify-between border-b pb-2">
                          ...
                          <span className="text-[10px] text-zinc-450 font-bold">Est. Value: ${gft.price.toFixed(2)}</span>
                        </div>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 text-[10px] text-zinc-500 font-semibold leading-relaxed">
                          ...
                          <div className="bg-zinc-100 dark:bg-zinc-900 p-2.5 rounded-lg border">
                            ...
                            <p className="italic text-zinc-650 dark:text-zinc-400 mt-0.5">"{gft.message.body}"</p>
                          </div>
                        </div>
                      </div>
                    ))}
              ...
              <div className="space-y-6 animate-in fade-in duration-200">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 items-center">
                  <div className="p-6 bg-zinc-50 dark:bg-zinc-900 border border-zinc-150 rounded-2xl flex flex-col items-center justify-center text-center shadow-xs">
                    <Award className="w-12 h-12 text-primary mb-3 animate-bounce" />
              ...
              <div className="space-y-6 animate-in fade-in duration-200">
                ...
                <div className="space-y-4">
                  {subscriptions.map((sub) => (
                    <div
                      key={sub.id}
                      className="p-5 rounded-2xl border border-zinc-150/60 dark:border-zinc-850 bg-zinc-50/50 dark:bg-zinc-900/30 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 hover:border-primary/20 transition"
                    >
                      <div className="space-y-1">
                        ...
                        <div className="text-[10px] text-zinc-450 font-semibold space-y-0.5 pt-1">
                      ...
                      <button
                        ...
                        className={`px-4 py-2 text-xs font-bold rounded-xl shadow-xs transition duration-200 transform hover:-y-0.5 cursor-pointer select-none ${
                          sub.status === "Active"
            ...
            {activeTab === "notifications" && (
              <div className="space-y-6 animate-in fade-in duration-200">
                ...
                <div className="space-y-3">
                  <div className="p-4 rounded-xl border border-blue-100 bg-blue-50/50 dark:bg-blue-955/20 text-xs font-semibold text-zinc-700 dark:text-zinc-300 flex items-start gap-3">
                  ...
                  <div className="p-4 rounded-xl border border-amber-100 bg-amber-50/50 dark:bg-amber-955/20 text-xs font-semibold text-zinc-700 dark:text-zinc-300 flex items-start gap-3">
            ...
            {activeTab === "settings" && (
              <div className="space-y-6 animate-in fade-in duration-200">
                ...
                <div className="max-w-md space-y-4">
                  <div>
                    <label className="block text-[10px] text-zinc-450 font-bold uppercase tracking-wider mb-1.5">Language preferences</label>
                    <select className="w-full px-3 py-2 border rounded-xl text-xs bg-zinc-50 dark:bg-dark text-zinc-900 dark:text-zinc-50">
                      <option>English (Global Sourcing)</option>
                      <option>Bengali (বাংলা)</option>
                    </select>
                  </div>
                  <div>
                    <label className="block text-[10px] text-zinc-450 font-bold uppercase tracking-wider mb-1.5">Corporate Currency Units</label>
                    <select className="w-full px-3 py-2 border rounded-xl text-xs bg-zinc-50 dark:bg-dark text-zinc-900 dark:text-zinc-50">
                  ...
                  <button
                    ...
                    className="px-5 py-2.5 rounded-xl bg-primary text-white text-xs font-bold transition shadow-xs"
                  >
                    Save Options
                  </button>
                </div>
